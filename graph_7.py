...
def read_files(file_path):
    ...
    with open(os.path.expanduser(file_path), 'r',encoding='GBK') as fp:
        fp=fp.readlines()
...
def get_one_policy_ratio_list(policy_ratio):
    val_acc1_list2=[]
    for i in range(3):
        file_path=f"./results/wrn16_2_from_wrn40_2_2倍_policy_{policy_ratio}_{i+1}.txt"
        print(file_path)
...
ax.plot(x, mean_average,"*--", markersize=6, label='Curve on CIFAR-100',c='r', linewidth=2)
...
plt.yticks([round(i,2) for i in np.linspace(73.7,76.9,6)])
plt.xlim(-0.1,5.1)
plt.ylim(73.7,76.9)
plt.tight_layout()
